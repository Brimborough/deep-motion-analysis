...
class Model(Container):
    ...
    def _fit_loop(self, f, ins, out_labels=[], batch_size=32,
        ...
        #For malik
        noise = 0
        noise_epochs = [500,1500,3000,4500,6000,7500]
        noise_list = [0.01,0.05,0.1,0.2,0.3,0.5]
        ...
        for epoch in range(nb_epoch):
            ...
            #Add noise like Malik
            if epoch in noise_epochs:
            ...
            for batch_index, (batch_start, batch_end) in enumerate(batches):
                ...
                try:
                    if type(ins_noise[-1]) is float:
                        # do not slice the training phase flag
                        ins_batch = slice_X(ins_noise[:-1], batch_ids) + [ins_noise[-1]]
                    else:
                        ins_batch = slice_X(ins_noise, batch_ids)
                except TypeError:
